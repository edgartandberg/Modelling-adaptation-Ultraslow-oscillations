...
a_k = 0.0e-9; % adaptation coupling
b_k = 2.0e-6; % adaptation gain
...
u_r = -55; % voltage reset
u_rest = -70; % resting potential
a_k_values = 0:1e-9:10e-9; % a_k from 0.0 to 10e-9
b_k_values = 5e-6:-0.5e-6:0; % b_k from 0 to 100
u_th= -50; % spike threshold
R = 10^7; %Resistance
A = 1; % toggle for adaptation, 1 for on 0 for off
...
for i = 1:length(a_k_values)
    ...
    for j = 1:length(b_k_values)
        ...
        heatmatrix_2(j,i) = heat_firing_rates; 
        r_array = [r_array heat_firing_rates];
        heat_spk_times_all{j,i} = heat_spk_times;
    end
...
u_rest = -70; % resting potential
a_k = 0.0; 
t_k_values = 10:10:100; 
b_k_values = 5e-6:-0.5e-6:0.5e-6; 
u_th= -50; % spike threshold
R = 10^7; %Resistance
A = 1; % toggle for adaptation, 1 for on 0 for off
