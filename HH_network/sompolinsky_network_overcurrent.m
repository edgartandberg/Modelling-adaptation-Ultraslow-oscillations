%% Sompolinsky network, over input current
...
all_results_sequences_e = {};


for i = 1:length(spike_times_e_all(1:end,1))
    disp(i)
    spike_times_e = [];
    spike_times_e = spike_times_e_all(i, :);
    spikes_bursts_e = [];
    ...
    for i = 1:length(spike_times_e{1, size_network})-1
        if (spike_times_e{1, size_network}(i+1)) - (spike_times_e{1, size_network}(i)) <= 4000  
            ...
            for spike_idx = 1:length(bursts_e{i})
                ...
                if valid_sequence
                    ...
                    if neuron_idx == length(spike_times_e)
                        ...
                        last_spike_time = spike_times_e{1,size_network}(final_bursts_e{i}(end));
                    end
                end
            end
    ...
    all_results_sequences_e{end + 1} = results_sequences_e; 
end


%% plotting sequence duration as a function of input current

% Load the Excel file
data = readtable('sequence durations excitatory, simulation over input current.xlsx');
...
title('Excitatory sequence durations as function of input current');
...
title('Average excitatory sequence duration as function of input current');
xlim([0 17]);
ylim([0 300]);
grid on;

% % Plot variances
% subplot(2, 1, 2); % Create a subplot for variances
% hold on;
% plot(x, variance2 * ones(size(x)), 'r--', 'LineWidth', 2, 'DisplayName', 'Variance Column 2');
% plot(x, variance3 * ones(size(x)), 'g--', 'LineWidth', 2, 'DisplayName', 'Variance Column 3');
% plot(x, variance4 * ones(size(x)), 'm--', 'LineWidth', 2, 'DisplayName', 'Variance Column 4');
% xlabel('Input Current');
% ylabel('Variance');
% title('Variance vs Input Current');
% legend show;
% grid on;
...
%% plotting inhibitory sequence duration as a function of input current
...
xlabel('Input Current [\muA/cm^2]');
ylabel('Sequence Durations [ms]');
title('Inhibitory sequence durations as function of input current');
...
xlabel('Input Current [\muA/cm^2]');
ylabel('Average sequence duration [ms]');
title('Average inhibitory sequence duration as function of input current');
xlim([0 17]);
ylim([0 300]);
grid on;
