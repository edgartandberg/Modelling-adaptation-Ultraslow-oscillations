...
title('Sequence durations as function of excitatory conductance, excitatory population');
...
% plot of spike diffs for e

% Create a 2x4 tiled layout
tiledlayout(3, 4);

% Loop through each subplot position
for i = 1:length(spike_diffs_e_all(:,1))
    nexttile; % Move to the next tile
    % Example data for plotting (replace with your actual data)
    plot(cell2mat(spike_diffs_e_all(i,:)), '-*', 'Color', 'k', 'LineStyle', '-'); 
    title(['Conductance =', num2str(conductance_array(i))]); % Title for each subplot

        % Add x and y axis labels
    xlabel('Sequence index'); % Replace with your desired label
    ylabel('Sequence duration [ms]'); % Replace with your desired label
end
